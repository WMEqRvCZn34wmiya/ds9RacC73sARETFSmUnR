...
function truth_combinations(
    ...
    silent || begin
        println("Execution time estimate...")
    ...
    silent || println("Start gen of combinations...")
    silent || println("Total combinations gen: $num_combinations")
    ...
    contradictions = 0
    i_rand = 0
    update_interval = max(1, num_combinations ÷ 100) 
...
function verify_simplification(original::TwoLevelDNFFormula, simplified::TwoLevelDNFFormula)
    ...
    num_samples = min(1000, 2^nuberofatoms(original))  
