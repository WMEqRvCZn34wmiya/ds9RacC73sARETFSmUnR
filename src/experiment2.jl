...
for c in [1.0]
    ...
    datasets = ["car", "post-operative", "urinary-d1", "urinary-d2", "iris"]

    # Create CSV file directly and write a more detailed header
    open("evaluation_results_experiment2.csv", "w") do csv_file
        ...
        for dataset in datasets
            ...
            if (dataset == "car")
                f, model, start_time, features_train, labels_train, features_test, labels_test, features, labels = learn_and_convert(7, dataset, 3)
            elseif (dataset == "iris" || dataset == "urinary-d1" || dataset == "urinary-d2" || dataset == "post-operative")
                f, model, start_time, features_train, labels_train, features_test, labels_test, features, labels = learn_and_convert(3, dataset, 3)
            else
            ...
            # Removed include("apiIntrees.jl") from here
            x = features_train
            y = labels_train
            ...
            timerefne = @elapsed begin
                nf = REFNE.refne(f, rangeXmin, rangeXmax, L=3, max_depth=1000) 
            end
            ...
            open("output_BestNumberOfTree_$dataset.txt", "w") do output_file
