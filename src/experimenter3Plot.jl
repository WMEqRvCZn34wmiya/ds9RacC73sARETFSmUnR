...
for c in  0.20:0.20:1.0
    
    datasets = ["iris", "hayes-roth", "monks-1", "monks-2", "monks-3", "balance-scale", "car", "post-operative", "urinary-d1", "urinary-d2"]
    
    # Create CSV file directly and write a more detailed header
    open("evaluation_results_experiment_for_plot_c=$c.csv", "w") do csv_file
        ...
        #println("dsintrees", dsintrees)
            ...
            # Removed include("RULECOSIplus.jl") from here
            ...
            open("output_$dataset.txt", "w") do output_file
                ...
                for (name, algorithm) in algorithms
                    ...
                    for (rule_id, eval) in enumerate(rule_evaluations)
                        ...
                        rule_idd = string(consequent(SoleModels.rules(algorithm)[rule_id]))
                        num_atoms = natoms(antecedent(rule))
                        
                        # Write all metrics to the CSV file for this rule
                        println(csv_file, "$dataset,$name,$num_terms,$exec_time,$rule_id,$sensitivity,$specificity,$min_avg,$min_std,$max_avg,$avg_avg,$std_avg,$num_atoms")
                    end
